...
void ofApp::update(){
    ...
    */
    ...
    countdown.update();
    if(frameCount % 12 == 0) { axisX.update(controls.activeKey.x); }
    if(frameCount % 12 == 6) { axisY.update(controls.activeKey.y); }
    

    //graphs
    if(frameCount % 60 == 0) { noise.update(ofMap(countdown.getSeconds(), 0, 60, 0, 1920)); }
